-- Kullanıcı isteği (26.08.2026): veli akışındaki gibi öğrenci/öğretmen
-- kaydında da tam bir KVKK Aydınlatma Metni onayı isteniyor ve bu onay
-- kalıcı olarak (versiyon + zaman damgası) kaydediliyor.
-- kvkk_onay_at/kvkk_onay_versiyon sütunları zaten vardı (migration 0006)
-- — o zamana kadar sadece veli akışında (/api/veli/tamamla) doldu-
-- ruluyordu. handle_new_user() trigger'ının son hali (migration 0052)
-- burada AYNEN korunuyor, sadece signUp sırasında gönderilen
-- kvkk_onay_versiyon metadata'sı artık profiles'a da yazılıyor — role'den
-- bağımsız, generic bir alan olduğu için tüm insert yollarında (öğrenci/
-- öğretmen/müdür/veli) aynı satırdan geçiyor.
...
declare
  ...
  v_kvkk_versiyon text := new.raw_user_meta_data->>'kvkk_onay_versiyon';
begin
  ...
  insert into public.profiles (id, ad, email, telefon, role, gecici_sifre, kvkk_onay_at, kvkk_onay_versiyon)
  values (
    new.id, v_profile_ad, new.email, new.raw_user_meta_data->>'telefon', v_role, v_gecici_sifre,
    case when v_kvkk_versiyon is not null then now() else null end, v_kvkk_versiyon
  );
