...
export function AnaSayfaDuyuruYonetimi({baslangic}:{baslangic:AnaSayfaDuyurusu[]}){
 ...
 return <div className="mt-4 rounded-3xl p-5" style={{background:BG1,border:`2px solid ${BORDER}`}}>
  ...
  <div className="sfec-liste mt-3">{liste.map(d=><div key={d.id} className="sfec-liste-satiri flex items-center gap-3 px-2 py-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold" style={{color:TEXT}}>{d.baslik}</p><p className="truncate text-xs" style={{color:TEXT_MUTED}}>{d.icerik}</p></div><button title="Düzenle" aria-label={`${d.baslik} duyurusunu düzenle`} onClick={()=>duzenle(d)}><Pencil size={16} color={MINT}/></button><button title="Sil" aria-label={`${d.baslik} duyurusunu sil`} onClick={()=>sil(d)}><Trash2 size={16} color={BLUSH}/></button></div>)}</div>
 </div>;
}
