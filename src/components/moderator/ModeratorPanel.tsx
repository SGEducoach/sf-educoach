...
export function ModeratorPanel({ okulAdi, kullanicilar }: { okulAdi: string; kullanicilar: ModeratorKullanici[] }) {
  ...
  return <div className="flex flex-col gap-5">
    <div className="rounded-3xl p-5" style={{ background: BG1, border: `2px solid ${BORDER}` }}>
      ...
      <Link href="/dashboard" className="sfec-btn mt-4 inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-xs font-bold" style={{ color: TEXT, border: `2px solid ${BORDER_STRONG}` }}><LayoutDashboard size={14}/>Ana sayfaya dön</Link>
    </div>
    ...
    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
      {sayfadakiler.map(k => <div key={k.id} className="rounded-2xl p-3.5" style={{ background: BG1, border: `2px solid ${BORDER}` }}>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <button disabled={pending} onClick={() => startTransition(async () => { const r = await moderatorAktiflikDegistir(k.id, !k.aktif); setMesaj(r.error ? `Hata: ${r.error}` : "İşlem tamamlandı."); })} className="sfec-btn flex-1 rounded-lg px-2 py-2 text-[11px] font-bold" style={{ color: k.aktif ? BLUSH : MINT, border: `2px solid ${BORDER_STRONG}` }}>{k.aktif ? <UserX className="mr-1 inline" size={12}/> : <UserCheck className="mr-1 inline" size={12}/>} {k.aktif ? "Pasifleştir" : "Aktifleştir"}</button>
          <button disabled={pending} onClick={() => { if (!window.confirm(`${k.ad} hesabı kalıcı olarak silinsin mi?`)) return; startTransition(async () => { const r = await moderatorHesapSil(k.id); setMesaj(r.error ? `Hata: ${r.error}` : "Hesap silindi."); }); }} className="sfec-btn rounded-lg px-3 py-2 text-[11px] font-bold" style={{ color: BLUSH, border: `2px solid ${BORDER_STRONG}` }}><Trash2 className="mr-1 inline" size={12}/>Sil</button>
        </div>
      </div>)}
