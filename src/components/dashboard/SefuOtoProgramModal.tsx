          ...
          {basari ? (
            ...
            <div className="mx-auto grid max-w-2xl gap-4">
              ...
              <fieldset>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(["haftalik", "aylik"] as ProgramKapsami[]).map((k) => <button type="button" key={k} onClick={() => { setKapsam(k); setVeri(null); }} className="sfec-btn rounded-2xl p-3 text-left" style={{ background: kapsam === k ? MINT_BG : BG1_ALT, border: `2px solid ${kapsam === k ? MINT : BORDER_STRONG}`, color: TEXT }}><span className="block text-sm font-extrabold">{k === "haftalik" ? "Haftalık uygula" : "Aylık uygula"}</span><span className="mt-1 block text-[11px]" style={{ color: TEXT_MUTED }}>{k === "haftalik" ? "Seçilen haftayı planlar." : "Dört haftalık iskeleti kurar; konular ilerler."}</span></button>)}
                </div>
              </fieldset>
              <Alan className="flex items-start gap-3"><Sparkles className="mt-0.5 shrink-0" size={17} color={PEACH} /><p className="text-xs leading-relaxed" style={{ color: TEXT_MUTED }}>SeFu zayıf ve yarım kalan konuları öne alır, öğretmen ödevlerini boş saatlere yerleştirir ve aynı dersi tek güne yığmaz. Programı kaydetmeden önce bütün kalemleri değiştirebilirsiniz.</p></Alan>
            </div>
          ) : adim === 1 && veri ? (
            ...
            <div className="grid gap-4">
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                ...
                <Alan><div className="text-[10px] font-bold" style={{ color: TEXT_MUTED }}>Konu / soru</div><div className="mt-1 text-lg font-extrabold" style={{ color: TEXT }}>{bloklar.filter((b) => b.tur === "konu").length} / {bloklar.filter((b) => b.tur === "soru").length}</div></Alan>
              ...
              <div className="overflow-x-auto rounded-2xl" style={{ border: `1px solid ${BORDER}` }}>
                <table className="w-full min-w-[760px] border-collapse text-left">
                  <thead style={{ background: BG1_ALT, color: TEXT_MUTED }}><tr>{["Tarih", "Saat", "Tür", "Ders", "Konu", ""].map((h) => <th key={h} className="px-2 py-2 text-[10px] font-extrabold uppercase tracking-wide">{h}</th>)}</tr></thead>
                  <tbody>{bloklar.map((b) => <tr key={b.anahtar} style={{ borderTop: `1px solid ${BORDER}` }}>
                    ...
                    <td className="p-2"><select disabled={Boolean(b.atamaId)} value={b.ders} onChange={(e) => bloguDegistir(b.anahtar, { ders: e.target.value, konu: veri.konuKuyruklari[e.target.value]?.[0] ?? null })} className="w-32 rounded-lg px-1.5 py-1.5 text-[10px] outline-none disabled:opacity-70" style={{ background: BG0, color: TEXT, border: `1px solid ${BORDER}` }}>{veri.dersListesi.map((d) => <option key={d}>{d}</option>)}</select></td>
                    <td className="p-2"><input disabled={Boolean(b.atamaId)} value={b.konu ?? ""} onChange={(e) => bloguDegistir(b.anahtar, { konu: e.target.value || null })} placeholder="Konu otomatik seçilmedi" className="w-44 rounded-lg px-2 py-1.5 text-[10px] outline-none disabled:opacity-70" style={{ background: BG0, color: TEXT, border: `1px solid ${BORDER}` }} /></td>
