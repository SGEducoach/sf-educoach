...
const TURLER: { id: YarismaTuru; ad: string }[] = [
  { id: "proje", ad: "Proje" }, { id: "yarisma", ad: "Yarışma" }, { id: "program", ad: "Program" },
];
  ...
  return <div className="space-y-5">
    ...
    <form id="sosyal-gorev-formu" onSubmit={kaydet} className="rounded-2xl p-4 sm:p-5" style={{ background: BG0, border: `1px solid ${duzenlenenId ? MINT : BORDER}` }}>
      ...
      {atamaYapabilir && <div className="mt-4 rounded-2xl p-3" style={{ background: BG1, border: `1px solid ${BORDER}` }}>
        ...
        <div className="grid gap-2 sm:grid-cols-2">{ogretmenler.map((ogretmen) => <label key={ogretmen.id} className="flex cursor-pointer items-center gap-2 rounded-xl p-2.5 text-sm" style={{ background: BG1_ALT, color: TEXT, border: `1px solid ${secili.includes(ogretmen.id) ? MINT : BORDER_STRONG}` }}><input type="checkbox" checked={secili.includes(ogretmen.id)} onChange={() => sec(ogretmen.id)} /><span className="min-w-0"><strong>{ogretmen.ad}</strong><span className="block truncate text-[10px]" style={{ color: TEXT_MUTED }}>{ogretmen.brans}</span></span></label>)}</div>
