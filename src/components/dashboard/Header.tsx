...
import { BORDER, BORDER_STRONG, LILAC_TEXT, MINT, MINT_BG, NAV_BG, TEXT, TEXT_MUTED } from "@/lib/theme";
...
export async function Header({ ad, role, kurumTuru, brans, okunmamisMesajSayisi = 0, mobilNavigasyon = true, moderatorMu = false, rolEtiketi, aktifBolum = "ozet", geriDonusHref, geriDonusEtiketi }: { ad: string; role: UserRole; kurumTuru?: KurumTuru; brans?: string; okunmamisMesajSayisi?: number; mobilNavigasyon?: boolean; moderatorMu?: boolean; rolEtiketi?: string; aktifBolum?: DashboardBolumu;
  ...
  return (
    ...
    <div className="sticky top-0 z-[100] isolate print:hidden">
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap sm:flex-nowrap">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-[10px] sm:text-[12px] italic truncate max-w-[150px] sm:max-w-none"><SeFuSlogan /></div>
              </div>
            </div>
      ...
      {yoneticiDuyurusu && (
        <div className="flex items-center gap-2 px-4 py-2 text-xs font-semibold" style={{
          background: "rgba(30, 25, 46, 0.85)", color: LILAC_TEXT,
          backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)",
          borderBottom: `2px solid ${BORDER_STRONG}`, boxShadow: "0 4px 14px rgba(0, 0, 0, 0.16)",
        }}>
